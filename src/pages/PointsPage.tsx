...
function RankingModule({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  return (
    <section className="points-module ranking-module">
      <div className="points-module-heading"><div><span className="points-eyebrow">POINTS LEADERBOARD</span><h2>积分排名</h2><p>按周期查看实习生星愿值排名与成长表现</p></div><button className="points-outline-button" type="button" onClick={() => onNotice('排名数据已准备导出')}>⇩ 导出排名</button></div>
...
function GiftModule({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  return (
    <section className="points-module gifts-module">
      <div className="points-module-heading"><div><span className="points-eyebrow">REWARD CATALOG</span><h2>礼品管理</h2><p>设置满足星愿值门槛即可兑换的招商银行相关礼品</p></div><button className="points-primary-button" type="button" onClick={openNewGift}>＋ 添加礼品</button></div>
...
function RedemptionModule({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  return (
    <section className="points-module redemption-module">
      <div className="points-module-heading"><div><span className="points-eyebrow">REDEMPTION TRACKING</span><h2>兑换记录</h2><p>按礼品汇总实习生兑换记录，方便线下统一发放</p></div><button className="points-outline-button" type="button" onClick={() => onNotice('兑换记录已准备导出')}>⇩ 导出记录</button></div>
